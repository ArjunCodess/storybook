...
export function createSnapshotReconciler(options: {
  ...
  return {
    ...
    tryAdoptEntry(incoming: EntryPayload): boolean {
      ...
      // Not recorded as seen, so a repaired replay can still apply it. Until then this writer's later
      // counters all count as gaps: applied, but the vector waits for PR 5's sync-request to repair.
