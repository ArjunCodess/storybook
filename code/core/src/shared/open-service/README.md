...
- [plain-object.ts](./plain-object.ts): the by-value copy every object takes on its way into state, the prototype-pollution key list, and the `hasOwn` / `isPlainObject` / `isReservedKey` guards shared by the recorder, apply-by-path, pointer schema, and snapshot merge
...
- `move`, `copy`, and `test`
- malformed `~` escapes
- `$`-prefixed segments, which deepsignal reserves for signal accessors
